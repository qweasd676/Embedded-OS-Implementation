    ...
    if (OSRunning == OS_TRUE) {
        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                ...
                //判斷是否為sort狀態，以及宣告taskID
                if (sortQ4 == 1 && sortQ2 == 0) {
                    if (OSPrioCur == 1) {
                        OSTCBCur->sortID = 3;
                    }
                    else if (OSPrioHighRdy == 1) {
                        OSTCBHighRdy->sortID = 3;
                    }

                    if (OSPrioCur == 2) {
                        OSTCBCur->sortID = 1;
                    }
                    else if (OSPrioHighRdy == 2) {
                        OSTCBHighRdy->sortID = 1;
                    }

                    if (OSPrioCur == 3) {
                        OSTCBCur->sortID = 2;
                    }
                    else if (OSPrioHighRdy == 3) {
                        OSTCBHighRdy->sortID = 2;
                    }
                }
                else if (sortQ2 == 1 && sortQ4 == 0) {
                    if (OSPrioCur == 1) {
                        OSTCBCur->sortID = 2;
                    }
                    else if (OSPrioHighRdy == 1) {
                ...
                if (OSPrioCur == 63) {
                    ...
                    OSTCBHighRdy->sortID = 63;
                }
                //-------------------------------------------//

                //判斷task目前是否會在這個tick做完，如果會做完，會直接做完到timedly()，不會經過interrupt的sw
                if (task2_time == execution_time2) {
                    OS_TRACE_ISR_EXIT();
                    OS_EXIT_CRITICAL();

                }
                //判斷說會不會在週期內無法做完，如果不會做完，會進入死迴圈
                else if (OSTimeGet() == 12 && task3_time < execution_time3) {
                    printf("%d\tMissDeadline\ttask( 2)( 0)\t------------\n", OSTimeGet());
                    while (1) {
                    }
                }
                else
                {
                    ...
                    if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                        ...
                        //計算每個task contextswitch
                        if (OSPrioCur == 1 || OSPrioHighRdy == 1 && OSTimeGet() > 0) {
                            ++task1_contextswitch;
                        }
                        if (OSPrioCur == 2 || OSPrioHighRdy == 2 && OSTimeGet() > 0) {
                            ++task2_contextswitch;
                        }
                        //顯示目前的task狀態、response time、contextswitch、下個切進來的task
                        if (OSPrioCur == 63) {
                            printf("%2d\tPreemption\ttask(%2d)     \ttask(%2d)(%2d)\n", OSTimeGet(),
                                OSPrioCur, OSTCBHighRdy ->sortID, OSTCBHighRdy->job_ID);
                        }
                        else
                        {
                            printf("%2d\tPreemption\ttask(%2d)(%2d)\ttask(%2d)(%2d)\n", OSTimeGet(),
                                OSTCBCur->sortID, OSTCBCur->job_ID, OSTCBHighRdy->sortID, OSTCBHighRdy->job_ID);
                        }
...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == OS_TRUE) {
        ...
        if (OSPrioCur == 2) {
            task2_time = task2_time + 1;
            //printf("\ntick:%d 中斷拉%d\n", OSTimeGet(), task2_time);
        }
    ...
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy == OSPrioCur) {
                ...
                ++task2_contextswitch;
                task2_response_time = task1_response_time * execution_time1 + execution_time2;
                printf("%2d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t%d\t\t%d\n", OSTimeGet(), OSPrioCur, OSTCBCur->job_ID,
                    OSPrioHighRdy, OSTCBCur->job_ID +1, task2_response_time, task2_contextswitch);
                OSTCBHighRdy->job_ID += 1;

                task1_response_time = 0;
                task2_contextswitch = 1;
                //判斷OSPrioHighRdy 是否為 OSPrioCur ，接續著做，但是還是要復歸。
                OS_TASK_SW();
            }
            else {
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                    ...
                    //---------------------------------------------//
                    //去計算每個task的contextswitch。
                    OSTCBCur->task_stop = OSTimeGet();
                    before_time = OSTimeGet();
                    if ((OSPrioCur == 1 || OSPrioHighRdy == 1) && OSTimeGet() > 0) {
                        ++task1_contextswitch;
                    }
                    if ((OSPrioHighRdy == 2 || OSPrioCur == 2) && OSTimeGet() > 0) {
                        ++task2_contextswitch;
                    }
                    if ((OSPrioHighRdy == 3 || OSPrioCur == 3) && OSTimeGet() > 0) {
                        ++task3_contextswitch;
                        ++task3_time;
                    }
                    //去計算每個task的contextswitch。
                    ...
                    if (OSPrioCur == 1 && OSTimeGet() > 0) {
                        task1_response_time++;
                        printf("%2d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t%d\t\t%d\n", OSTimeGet(), OSTCBCur -> sortID, OSTCBCur->job_ID,
                            OSTCBHighRdy->sortID, OSTCBHighRdy->job_ID, (OSTCBCur->task_stop - OSTCBCur->task_start), task1_contextswitch);
                        task1_contextswitch = 0;
                    }
                    else if (OSPrioCur == 2)
                    {
                        task2_response_time = task1_response_time * execution_time1 + execution_time2;
                        ...
                        if (OSPrioHighRdy == 63) {
                            ...
                            printf("%2d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)  \t%d\t\t%d\n", OSTimeGet(), OSTCBCur->sortID, OSTCBCur->job_ID,
                                OSTCBHighRdy -> sortID, task2_response_time, task2_contextswitch);
                            task2_contextswitch = 0;
                            task1_response_time = 0;
                        }
                        else
                        {
                            printf("%2d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t%d\t\t%d\n", OSTimeGet(), OSTCBCur->sortID, OSTCBCur->job_ID,
                                OSTCBHighRdy->sortID, OSTCBHighRdy->job_ID, task2_response_time,task2_contextswitch);
                            task2_contextswitch = 0;
                            task1_response_time = 0;
                        }
                    }
                   ...
                   INT16U   opt)
{
    ...
    if (ptcb != (OS_TCB *)0) {
        ...
        ptcb->OSTCBDly           = 2u;                     /* Task is not delayed                      */
#if OS_TASK_CREATE_EXT_EN > 0u
