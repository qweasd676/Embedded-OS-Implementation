    ...
    if (OSRunning == OS_TRUE) {
        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                ...
                //判斷已經做完但是會被搶暫的問題
                if (OSTCBCur->execution_frequency == OSTCBCur->execution && OSPrioCur != OS_TASK_IDLE_PRIO ) {
                    //printf("OSTCBCurid: %d OSTCBCur->execution_frequency %d , OSTCBCur->execution %d \n", OSTCBCur->OSTCBId,OSTCBCur->execution_frequency, OSTCBCur->execution);
                    OS_TRACE_ISR_EXIT();
                }
                else {
                    ...
                    if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                        ...
                        // OSIntExit (void)中計算每個被中斷的task contextswitch
                        if (OSTimeGet() > OSTCBCur->task_arrival) {
                            if (OSPrioCur == 63) {
                                printf("%2d\tPreemption\ttask(%2d)     \ttask(%2d)(%2d)\n", OSTimeGet(),
                                    OSPrioCur, OSTCBHighRdy->OSTCBId, OSTCBHighRdy->job_ID);
                            }
                            else
                            {
                                printf("%2d\tPreemption\ttask(%2d)(%2d)\ttask(%2d)(%2d)\n", OSTimeGet(),
                                    OSTCBCur->OSTCBId, OSTCBCur->job_ID, OSTCBHighRdy->OSTCBId, OSTCBHighRdy->job_ID);
                            }
...
void  OSStart (void)
{
    ...
    OS_TCB* tcb_ = OSTCBList;
    if (OSRunning == OS_FALSE) {
        //製作 TCB linked list 表
        //printf("==================== TCB linked list ====================\n");
        //printf("Task\tPrev_TCB_addr\tTCB_addr\tNext_TCB_addr\n");
        //while (tcb_ > 0)
        //{
        //    printf("%d\t%08X\t%08X\t%08X\n", tcb_->OSTCBPrio, tcb_->OSTCBPrev, tcb_, tcb_->OSTCBNext);
        //    tcb_ = tcb_->OSTCBNext;
        //}
        printf("\nTick\tEvent\t     CurrentTask ID\tNextTask ID\tResponseTime\t # of ContextWsitch\n");
        //製作 TCB linked list 表
        //y             = OSUnMapTbl[OSRdyGrp];                           //尋找最高權限
        //OSPrioHighRdy = (INT8U)((y << 3u) + OSUnMapTbl[OSRdyTbl[y]]);   //尋找最高權限
        start = 1;
...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == OS_TRUE) {
        ...
        //檢查每個已經有ready但沒執行的task,此程式在OSTimeTick()執行
        for (INT8U y = 0; y < 8; y++) {
            for (INT8U x = 0; x < 8; x++) {
                if ((OSRdyTbl[y] & (INT8U)1 << x) == (INT8U)1 << x) {   //確認是否有ready
                    INT8U prio = (y << 3u) + x;
                    if (OSTCBPrioTbl[prio]->OSTCBId == 4 && OSTCBPrioTbl[prio]->job_ID >= 2) {
                        prio = OS_TASK_IDLE_PRIO;
                    }
                    if (OSTCBPrioTbl[prio] != OSTCBCur && prio != OS_TASK_IDLE_PRIO) {
                        if (OSTimeGet() > OSTCBPrioTbl[prio]->task_arrival) {
                            //每經過一個ticktime,其他有ready的task的反應時間都會+1，並每次調整EDF_Deadline時間
                            OSTCBPrioTbl[prio]->response += 1;
                            OSTCBPrioTbl[prio]->EDF_deadline = OSTCBPrioTbl[prio]->Deadline - OSTCBPrioTbl[prio]->response;
                            //printf("time %d task %d EDF %d\n", OSTimeGet(), OSTCBPrioTbl[prio]->OSTCBId, OSTCBPrioTbl[prio]->EDF_deadline);

                            if (OSTCBPrioTbl[prio]->response  > OSTCBPrioTbl[prio]->Deadline) {
                                printf("%d\tMissDealine\tTask( %d)( %d)\t----------\n", OSTimeGet(),
                                    OSTCBPrioTbl[prio]->OSTCBId, OSTCBPrioTbl[prio]->job_ID);
                                OSTCBPrioTbl[prio]->EDF_deadline_break = 1;
                            }
                        }
                        else if (OSTimeGet() >= OSTCBPrioTbl[prio]->task_arrival) {
                            //printf("dsadsa\n");
                            OSTCBPrioTbl[prio]->EDF_deadline = OSTCBPrioTbl[prio]->Deadline;
                        }
                    }              
                }
            }
        }
        //每經過一個ticktime,OSTCBCur的工作次數跟反應時間都會+1，並每次調整EDF_Deadline時間
        if (OSPrioCur != OS_LOWEST_PRIO) {
            //printf("%d OSPrioCur%d\n", OSTimeGet(),OSPrioCur);
            OSTCBCur->response += 1;
            OSTCBCur->execution_frequency += 1;     //經過一個ticktime ++執行時間
            OSTCBCur->EDF_deadline = OSTCBCur->Deadline - OSTCBCur->response;

        }
        //檢查是否有missdeadline
        //printf("time %d OSTCBCur->response %d, OSTCBCur->Deadline %d \n", OSTimeGet(), OSTCBCur->response, OSTCBCur->Deadline);
        if (OSTCBCur->response >= OSTCBCur->Deadline && (OSTCBCur->execution - OSTCBCur->execution_frequency > 0)) {
            if (OSTCBCur->OSTCBId == 4) {  //判斷非週期MissDeadline
                printf("%2d\tAperiodic job (%d) misses deadline.",OSTimeGet(), OSTCBCur->job_ID);
            }
            else {
                printf("%2d\tMissDealine\tTask( %d)( %d)\t------------\n", OSTimeGet(),
                    OSTCBCur->OSTCBId, OSTCBCur->job_ID);
            }
            OSTCBCur->EDF_deadline_break = 1;
        ...
        //                    printf("check time %d task %d EDF %d  re%d  exc%d \n", OSTimeGet(), OSTCBPrioTbl[prio]->OSTCBId, OSTCBPrioTbl[prio]->EDF_deadline, OSTCBPrioTbl[prio]->response, OSTCBPrioTbl[prio]->execution_frequency);
...
void  OS_Sched (void)
{
    ...
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            OS_SchedNew();
            OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];
            //printf("OSTCBCur->EDF_deadline_break: %d\n", OSTCBCur->EDF_deadline_break);
            //OS_Sched(void)中如果當下個task也是同一個taks的處理方式
            if (OSPrioHighRdy == OSPrioCur && OSTCBCur->EDF_deadline_break != 1) {
#if OS_TASK_PROFILE_EN > 0u
                OSTCBHighRdy->OSTCBCtxSwCtr++;         /* Inc. # of context switches to this task      */
#endif
                ...
                OS_TLS_TaskSw();
#endif
#endif          //判斷OSPrioHighRdy 是否為 OSPrioCur ，接續著做，但是還是要復歸。
                if (OSTimeGet() > OSTCBCur->task_arrival) {
                    OSTCBCur->contextswitch += 1;
                    printf("%2d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t%d\t\t%d\n", OSTimeGet(), OSTCBCur->OSTCBId, OSTCBCur->job_ID,
                          OSTCBHighRdy->OSTCBId, OSTCBHighRdy->job_ID, OSTCBCur->response, OSTCBCur->contextswitch);
                    OSTCBCur->contextswitch = 0;
                    OSTCBCur->execution_frequency = 0;
                    OSTCBHighRdy->contextswitch += 1;
                    OSTCBCur->response = 0;
                    OSTCBCur->job_ID += 1;
                    OSTCBCur->EDF_deadline = OSTCBCur->Deadline;

                }
                OS_TASK_SW();
            }
            //OS_Sched(void)中task做完時的處理方式，並包含非週期處理完的方式
            else if(OSTCBCur->EDF_deadline_break != 1){
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
#if OS_TASK_PROFILE_EN > 0u
                    OSTCBHighRdy->OSTCBCtxSwCtr++;         /* Inc. # of context switches to this task      */
#endif
                    ...
                    if (OSTimeGet() > OSTCBCur->task_arrival) {
                        ...
                        OSTCBHighRdy->contextswitch += 1;
                        OSTCBCur->contextswitch += 1;
                        if (OSTCBCur->OSTCBId == 4) {
                            printf("%2d\tAperidoic job (%d) is finished.\n", OSTimeGet(), OSTCBCur->job_ID);
                        }
                        if (OSPrioHighRdy == 63) {
                            printf("%2d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)  \t%d\t\t%d\n", OSTimeGet(), OSTCBCur->OSTCBId, OSTCBCur->job_ID,
                                OSPrioHighRdy, OSTCBCur->response, OSTCBCur->contextswitch);
                        }
                        else {
                            printf("%2d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t%d\t\t%d\n", OSTimeGet(), OSTCBCur->OSTCBId, OSTCBCur->job_ID,
                                            OSTCBHighRdy->OSTCBId, OSTCBHighRdy->job_ID, OSTCBCur->response, OSTCBCur->contextswitch);
                        ...
                        OSTCBCur->job_ID += 1;
                        OSTCBCur->EDF_deadline = OSTCBCur->Deadline;
                        //當第一個非週期的task做完,必須馬上給下一個非週期的參數
                        if (OSTCBCur->OSTCBId == 4) {
                            if (Aperiodic_Arrival_job2 < Aperiodic_period_job1) {
                                OSTCBCur->task_arrival = Aperiodic_period_job1;
                            }
                            else {
                                OSTCBCur->task_arrival = Aperiodic_Arrival_job2;
                            }
                            OSTCBCur->execution = Aperiodic_exection_job2;
                            OSTCBCur->Deadline = Aperiodic_period_job2 - OSTCBCur->task_arrival;
                            OSTCBCur->EDF_deadline = OSTCBCur->Deadline;
                            //printf("id %d arrival %d edf_deadline %d \n", OSTCBCur->OSTCBId, OSTCBCur->task_arrival, OSTCBCur->EDF_deadline);
                        }

                    }
                    //顯示task目前的response time、ontextswitch、工作多少次以及目前task狀態和下一個執行的task

                    OS_TASK_SW();                          /* Perform a context switch                     */
                }

            }
            //OS_Sched(void)中如果此task missdeadline的後續處理
            else {
                OSTCBCur->contextswitch = 0;
                OSTCBCur->execution_frequency = 0;
                OSTCBCur->response = 0;
...
static  void  OS_SchedNew (void)
{
    ...
    //y             = OSUnMapTbl[OSRdyGrp];                           //尋找最高權限
    //OSPrioHighRdy = (INT8U)((y << 3u) + OSUnMapTbl[OSRdyTbl[y]]);   //尋找最高權限 OSPrioHighRdy
    if (OSTimeGet() >= 1) {
        start = 0;
    }
    //一開始進task需要找到哪個task最小,但會skip還沒arrival的task
    if (start == 1) {
        INT8U deadline_test = 10;
        for (INT8U y = 0; y < 8; y++) {
            for (INT8U x = 0; x < 8; x++) {
                if ((OSRdyTbl[y] & (INT8U)1 << x) == (INT8U)1 << x) {   //確認是否有ready
                    INT8U prio = (y << 3u) + x;
                    if (prio != OS_TASK_IDLE_PRIO && OSTimeGet() >= OSTCBPrioTbl[prio]->task_arrival) {
                        //printf("time %d dasdas%d\n", OSTimeGet(),OSTCBPrioTbl[prio]->OSTCBId);
                        if (OSTCBPrioTbl[prio]->Deadline < deadline_test) {
                            deadline_test = OSTCBPrioTbl[prio]->Deadline;
                            OSPrioHighRdy = (INT8U)((y << 3u) + x);
    ...
    //判斷每個tick，誰的deadline最小,但需判斷每個task是否已經做完
    else if (start == 0)
    {
        //printf("2\n");
        INT8U idle_time = 0;
        ...
        for (INT8U y = 0; y < 8; y++) {
            for (INT8U x = 0; x < 8; x++) {
                if ((OSRdyTbl[y] & (INT8U)1 << x) == (INT8U)1 << x) {   //確認是否有ready
                    INT8U prio = (y << 3u) + x;
                    //printf("time %d ptcb %d\n", OSTimeGet(),OSTCBPrioTbl[prio]->OSTCBId);


                    //當非週期的task做完時，我強制讓他為OS_TASK_IDLE_PRIO,以及去檢查下個tick是否有非OS_TASK_IDLE_PRIO的task
                    if (OSTCBPrioTbl[prio]->OSTCBId ==4 && OSTCBPrioTbl[prio]->job_ID >= 2) { 
                        prio = OS_TASK_IDLE_PRIO;
                    }
                    else {
                    ...
                    //EDF的判斷式,主要分成當前task做完跟沒做完的處理,去尋找已經有ready的task判斷他們誰deadline最低
                    if ( prio != OS_TASK_IDLE_PRIO && OSTimeGet() >= OSTCBPrioTbl[prio]->task_arrival) {
                        ptcb = OSTCBPrioTbl[prio];
                        //printf("time %d next %d deadline %d cur  %d  deadline %d\n", OSTimeGet(), ptcb->OSTCBId,ptcb->EDF_deadline, hptcb->OSTCBId, hptcb->EDF_deadline);
                        if (OSTCBCur->execution_frequency != OSTCBCur->execution ) {
                            if (ptcb->EDF_deadline < hptcb->EDF_deadline) {
                                //printf("1\n");
                                hptcb = ptcb;
                                OSPrioHighRdy = (INT8U)((y << 3u) + x);
                            }
                        ...
                        else if(OSTCBCur->execution_frequency == OSTCBCur->execution ){
                            //hptcb = ptcb;
                            //printf("10\n");
                            if (ptcb->EDF_deadline == 0) {
                                //printf("11\n");
                                ptcb->EDF_deadline = ptcb->Deadline;
                            }

                            if (ptcb->EDF_deadline < EDF_test) {
                                //printf("4\n");
                                EDF_test = ptcb->EDF_deadline;
                                hptcb = ptcb;
                                OSPrioHighRdy = (INT8U)((y << 3u) + x);
                    ...
                    else if (prio == OS_TASK_IDLE_PRIO && idle_time ==0) {  //當下個tick都沒事情做會讓他等於OS_TASK_IDLE_PRIO
                        //printf("dadas\n");
                        OSPrioHighRdy = (INT8U)((y << 3u) + x);   //尋找最高權限
                    }
                }
            }
        }
    ...
    //printf("1\n");
    //printf(" time %d OSPrioHighRdy: %d\n",OSTimeGet(), OSTCBPrioTbl[OSPrioHighRdy]->OSTCBId);
                   ...
                   INT32U   arrival)
{
    ...
    if (ptcb != (OS_TCB *)0) {
        ...
        //ptcb->execution_time     = 0;
        //ptcb->execution_en       = 0;
        ptcb->job_ID = 0;
        ptcb->response = 0;
        ptcb->execution_frequency = 0;
        ptcb->contextswitch = 0;
        ptcb->Deadline = period;
        ptcb->task_arrival = arrival;
        ...
        //print出taks目前、上個、下個的TASK狀態
        //printf("\nTask[\t%d] create, TCB address %08X", prio, OSTCBPrioTbl[prio]);
        //printf("\n-------After TCB[%d] being linked------",ptcb->OSTCBPrio);
        //printf("\nPrevoius TCB point to address %08X", ptcb->OSTCBPrev);
        //printf("\nCurrent  TCB point to address %08X", OSTCBPrioTbl[prio]);
        //printf("\nNext     TCB point to address %08X\n",ptcb->OSTCBNext);
        //print出taks目前、上個、下個的TASK狀態
